...
| **Box3D Joint CHOP** (`Box3djointchop`) | CHOP | The joint authoring node. Connects bodies by path/name plus index and uses the joint pivot stored on each Body SOP: set the pivot on the bodies, then connect Body A and Body B (leave Body B empty to pin Body A to the world). The **Joints** parameter enables up to 8 Body A/Body B pair rows on the Bodies page (Constant CHOP style), and **Count** turns each pair into an index series for chains. The **Pivot** menu picks how the anchor is placed: each body's own pivot (default, pivot-to-pivot), one body's pivot for both sides (**Body B Pivot** is the child-bone convention — the child body carries the anchor, so bodies jointed on both ends articulate correctly with a single pivot each), or an explicit world-space **Anchor**. Outputs `ax ay az bx by bz active`, one sample per joint. |
